...
def main():
    print("="*70)
    print("🚀 MULTI-PLANE FUSION TRAINING")
    print("="*70)
    ...
    CONFIG = {
        ...
        'task': 'abnormal',
        'batch_size': 8,  # Smaller due to 3 planes
        'num_epochs': 30,
        'learning_rate': 2e-5,
        'weight_decay': 1e-3,
        'dropout_rate': 0.4,
    ...
    # Model
    print("\n🤖 Creating multi-plane model...")
    model = MultiPlaneFusion(
        num_classes=1,
    ...
    print("\n" + "="*70)
    print("🔥 Starting Multi-Plane Training")
    print("="*70)
    ...
    for epoch in range(CONFIG['num_epochs']):
        ...
        # Combined score: 40% AUC + 35% Sensitivity + 25% Specificity
        combined_score = 0.4 * val_auc + 0.35 * val_sens + 0.25 * val_spec
        ...
        if combined_score > best_score and val_spec >= CONFIG['min_specificity']:
            ...
            torch.save(checkpoint, f"{CONFIG['output_dir']}/models/best_model_multiplane.pth", weights_only=False)
    ...
    # AUC
    axes[0, 1].plot(epochs_range, history['train_auc'], 'b-', label='Train', linewidth=2)
    axes[0, 1].plot(epochs_range, history['val_auc'], 'r-', label='Val', linewidth=2)
    ...
    axes[1, 0].set_title('Sensitivity', fontsize=14, weight='bold')
    axes[1, 0].legend()
    ...
    axes[1, 1].axhline(y=0.65, color='orange', linestyle='--', label='Min Target')
    ...
    axes[1, 2].set_title('Learning Rate', fontsize=14, weight='bold')
    axes[1, 2].set_yscale('log')
    axes[1, 2].grid(alpha=0.3)
    
    plt.suptitle('MULTI-PLANE FUSION TRAINING', fontsize=18, weight='bold')
    plt.tight_layout()
    plt.savefig(f"{CONFIG['output_dir']}/plots/training_multiplane.png", dpi=200, bbox_inches='tight')
    plt.close()
    
    print(f"✅ Training plots saved")
    print("\n" + "="*70)
    print("🎉 MULTI-PLANE TRAINING COMPLETE!")
    print("="*70)
